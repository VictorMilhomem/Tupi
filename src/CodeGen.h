...
void print_program(const Assembly::Program& program, int level);
...
public:
    ...
    void instruction_generator(const Assembly::Instruction& instruction) {
        std::cout << "\tTODO: Generate Instructions " << std::endl;
    };


    void function_generator(const Assembly::FunctionDefinition& functiondef) {
        std::cout << "\t.globl " << functiondef.function->name << std::endl;
        std::cout << functiondef.function->name << ":" << std::endl;
        for (const auto& inst : functiondef.function->body) {
            instruction_generator(*inst);
        }
    }
    void program_generator() {
        auto prog = convert_program();
        std::cout << ";; Code Generated by Tupi Compiler" << std::endl;
        function_generator(*prog->function_def);
        std::cout << std::endl;
        std::cout << "	.ident	\"GCC: (x86_64-posix-seh-rev0, Built by MinGW-W64 project) 8.1.0\" " << std::endl;
    };
